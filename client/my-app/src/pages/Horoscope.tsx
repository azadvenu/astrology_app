...
import { useState } from "react";
import { Sparkles, History, MapPin, Clock, Calendar, Loader2 , User} from "lucide-react";
...
export default function Horoscope() {
  ...
  const [formData, setFormData] = useState({
    name: "", // You can get this from Redux state
    dob: "",
    tob: "",
    place: ""
  });
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // This calls your backend, which then calls the external Astrology API
      const res = await generateHoroscopeApi(formData);
      setHoroscopeData(res.data.result);
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#fdfaf5] font-sans">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-12">
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          {/* Input Form */}
          <section className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-yellow-100 h-fit">
            <h2 className="text-3xl font-serif font-bold text-slate-800 mb-6">Birth Details</h2>
            <form className="space-y-6" onSubmit={handleGenerate}>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold text-slate-600"><User size={16} />Full Name</label>
                  <input
                    type="text"
                    required
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="w-full p-4 rounded-2xl border border-yellow-100 bg-[#fdfaf5] outline-none focus:ring-2 focus:ring-[#c67605]"
                  />
                </div>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold text-slate-600"><Calendar size={16} /> Date of Birth</label>
              ...
              <button
                disabled={loading}
                className="w-full bg-[#c67605] text-white py-4 rounded-full font-bold shadow-lg hover:bg-[#a45c40] transition-all flex justify-center items-center gap-2"
              >
                {loading ? <Loader2 className="animate-spin" /> : "Generate My Chart"}
              </button>
            </form>
          </section>
          ...
          <section className="bg-[#5c4033] text-white p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="space-y-6">
                <div className="p-6 bg-white/5 rounded-3xl border border-white/10 min-h-[150px]">
                  {horoscopeData ? (
                    <p className="text-lg leading-relaxed italic text-yellow-50/90">
                      "{horoscopeData[activeTab] || horoscopeData.description}"
                    </p>
                  ) : (
                    <p className="text-white/40 italic text-center mt-10">Enter your details to reveal the stars...</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                    <span className="text-xs text-yellow-400 font-bold uppercase">Lucky Number</span>
                    <p className="text-2xl font-bold">{horoscopeData?.lucky_number || "--"}</p>
                  </div>
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                    <span className="text-xs text-yellow-400 font-bold uppercase">Lucky Color</span>
                    <p className="text-2xl font-bold">{horoscopeData?.lucky_color || "--"}</p>
                  </div>
                </div>
              </div>
            </div>
